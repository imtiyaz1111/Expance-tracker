...
const UpdateReviewModal = ({ show, onHide }) => {
  ...
  useEffect(() => {
    getUserReview(setGetReview);
  }, []);
  console.log("getUserReview2323", getReview);
  ...
  useEffect(() => {
    if (getReview?.data) {
      setReviewData({
        comment: getReview.data.comment || "",
        rating: getReview.data.rating || 0,
      });
    }
  }, [getReview]);
